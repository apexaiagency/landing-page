...
| 3 | ~~Footer contact address~~ | **CLOSED 15 Sep.** The footer now books a walkthrough instead of showing an address, and `contactEmail` is removed from content and schema. A published address still needs mail live on the domain, so put one back only when that is true |
| 3b | Header | **Sign-in URL** so the Log in button works | Button renders **disabled** rather than as a dead link, and the build prints a reminder | `content/landing-content.json → login.href` (replace `PLACEHOLDER_NOT_CONNECTED`). The platform subdomain agreed on 8 Sep was `desk.offsitelabs.io`, but confirm before using it |
